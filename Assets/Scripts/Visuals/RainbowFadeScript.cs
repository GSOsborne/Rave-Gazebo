...
public class RainbowFadeScript : MonoBehaviour
{
    ...
    IEnumerator ColorShiftRedToMagenta()
    {
        ...
        while (forever)
        {
            ...
            {
                rend.sharedMaterial.color = Color.Lerp(Color.red, Color.magenta, i / 100);
            ...
            for (float i = 0; i <= 40; i++)
            {
                rend.sharedMaterial.color = Color.Lerp(Color.magenta, Color.blue, i / 100);
                yield return new WaitForSeconds(frameLength);
            }
            //Debug.Log("Should have turned blue!");
            for (float i = 0; i <= 40; i++)
            {
                rend.sharedMaterial.color = Color.Lerp(Color.blue, Color.green, i / 100);
                yield return new WaitForSeconds(frameLength);
            }
            //Debug.Log("Should have turned green!");
            for (float i = 0; i <= 40; i++)
            {
                rend.sharedMaterial.color = Color.Lerp(Color.green, Color.yellow, i / 100);
                yield return new WaitForSeconds(frameLength);
            }
            //Debug.Log("Should have turned yellow!");
            for (float i = 0; i <= 40; i++)
            {
                rend.sharedMaterial.color = Color.Lerp(Color.yellow, Color.red, i / 100);
                yield return new WaitForSeconds(frameLength);
            }
